...
(function(factory){
  ...
  var
	...
	//--> FBDeepZoom Additional Definition
        zoomdelta:		20, //FBZoom steps to zoom in
        zoomlevel:		 0, //FBZoom current zoom level
	zoomspeed: 	       500, //FBZoom animation speed (ms)
	tileSize:	       256, //FBZoom tile size (pixel)
	overlap: 		 0, //FBZoom tiles overlap
	offset: 	       '0', //boundaries offset (px or %). If 0 image move side to side and up to down
	numLevels:	       101, //FBZoom Number of levels
	startLevel:	         0, //FBZoom start level
	folder:	       'deep_duck', //FBZoom image folder
	imagewidth:	       256, //FBZoom basic image width
	imageheight:	       226, //FBZoom basic image height
	zoomwidth:	      2048, //FBZoom zoom image width
	zoomheight:	      1806, //FBZoom zoom image height
	zoomcache:		[], //FBZoom zoom image already loaded?
	//<-- FBDeepZoom
          ...
          applicable.each(function(){
              ...
              on= {
                ...
                instance: {
                  ...
                  setup: function(e){
                    ...
                    //--> FBDeepZoom
                    //$overlay.parent().css({width: prozent(100), height: opt.viewportHeight})
                    t.parent().parent().css({width: opt.imagewidth, height: opt.imageheight, overflow:'hidden', position:'relative'});
                    initTilezoom($overlay, opt); //FBDeepZoom
                    //<-- FBDeepZoom
                    
                    opt.indicator && $overlay.parent().append(indicator('x')); //FBDeepZoom Append indicator on top of zoom
                    multirow && opt.indicator && $overlay.parent().append(indicator('y')); //FBDeepZoom Append indicator on top of zoom
                  ...
                  */
                  ...
                  pan: function(e, x, y, ev){
                    if (opt.draggable && slidable && !opt.inAction){ //FBDeepZoom: no pan while zooming (inAction)
                      slidable= false;
                      unidle();
                      var
                        rows= opt.rows,
			...
			if (!(fraction % 1) && !opt.loops) var
			  origin= recenter_mouse(revolution, x, y)
                  ...
                  //--> FBDeepZoom
                  zoom: function( e, zoomlevel, coords ) {
			if (zoomlevel < 0) zoomlevel = 0;
			else if (zoomlevel > 100) zoomlevel = 100;

			if(zoomlevel != opt.zoomlevel || (zoomlevel == opt.zoomlevel && opt.frame != get(_frame_))) {
				var $cont = t.parent();
				opt.frame = get(_frame_);
				if(opt.inAction) return false;

				opt.zoomlevel = zoomlevel;

				setSizePosition($cont, opt, coords, opt.zoomspeed, function() {
					initTiles($cont, opt, false);
				});
			}
			else {
				return false;
				...
				t.parent().mousewheel(function(e, delta) {
					var coords = {};
					coords.x = e.pageX;
					coords.y = e.pageY;

					zoomlevel = Math.round(opt.zoomlevel + (delta * opt.zoomdelta));
					t.trigger('zoom', [zoomlevel, coords]);
                  ...
                  frameChange: function(e, nil, frame){
                    ...
                    if (!is_sprite){
                      ...
                      //--> FBDeepZoom: frameChange during zoom
			if (opt.zoomlevel > 0) {
				if (typeof(opt.zoomcache[frame - 1]) == 'undefined') opt.zoomcache[frame - 1] = {};

				$('div#zoom-tiles img').each(function(){
					var tilenr = $(this).attr('id').substr(10);
					
					//if zoom tile is cached then just change otherwise load zoom tile
					if (opt.zoomcache[frame - 1][tilenr] !== true) {
						$(this).hide();
						var thisFrame = frame;
						var thisTile = $(this);

						var hiSrc = reen(opt.folder+'/'+(frame - 1)+'/'+tilenr+'.jpg');
						$imgZoom= $(tag(_img_)).attr({ src: hiSrc }).appendTo(dot(cache_klass));
						opt.zoomcache[frame - 1][tilenr] = true;

						setTimeout(function(){ 
							if (thisFrame == get(_frame_)) swapImage(thisTile, hiSrc);
						}, 500);

					} else {
						$(this).attr({ 
							src: reen(reel.substitute(opt.folder+'/'+(frame - 1)+'/'+tilenr+'.jpg', get))
						}).show();
					}
				});
			}
  ...
  //--> FBDeepZoom Several Functions to implement DeepZoom
  ...
  //init Tilezoom
  function initTilezoom($cont, settings) {
	settings.userAgent = navigator.userAgent.toLowerCase();
	settings.inAction = false;

	//container
	settings.cont = $cont;
	settings.deepzoom_min = {};
	settings.deepzoom_max = {};
  ...
  function buildMarkup($cont, settings) {
	$cont.parent().wrap('<div></div>').addClass('zoom-container').addClass('zoom-0');
	$cont.addClass('zoom-holder');

	var $holder = settings.holder = $cont;
	var $thumb = settings.thumb = $holder.children('img.reel');
	$holder.append('<div id="zoom-tiles" class="tile-0"></div>');
  ...
  function initNumLevels(settings) {
	var maxDimension = Math.max(settings.zoomwidth, settings.zoomheight);
	var numLevels = number(Math.ceil(Math.log(maxDimension/settings.tileSize)/Math.log(2)) + 1); //FB3D var numLevels = number(Math.ceil(Math.log(maxDimension)/Math.log(2)) + 1);
	return numLevels;
  }
  ...
  function initLevel(settings) {
	...
	var viewportWidth = settings.imagewidth;
	var viewportHeight = settings.imageheight;

	while(0 <= level && level < settings.numLevels) { //FB3D while(9 <= level
		var levelImage = getImage(level, settings);
		if(levelImage.width>=viewportWidth || levelImage.height>=viewportHeight) {
			break;
		}
		level++;
	}
  ...
  function initTiles($cont, settings, start) {
	var zoomlevel = settings.zoomlevel;
	var deeplevel = settings.deeplevel;
	var visibleTiles = getVisibleTiles($cont, settings);
	var newtile = settings.frame-1;
	if (settings.changesort) {
		newtile = settings.frames - newtile;
		if (newtile == settings.frames) newtile = 0;
	}

	var levelDir = settings.folder+'/'+(newtile)+'/'+deeplevel+'-';
	var $tiles = $cont.children('div#zoom-tiles'); //FB3D settings.tiles;
	document.getElementById('zoom-tiles').className= document.getElementById('zoom-tiles').className.replace(/tile-\d+/, 'tile-' + newtile);

	var dimension = getDimension(deeplevel, settings);
	var nowscale = (zoomlevel == 0) ? (settings.imagewidth/dimension.width) : settings.deepscale;
	var startpow = (Math.pow(2, deeplevel) - 1);
	var startwidth = {};
	startwidth[0] = Math.round(nowscale * settings.tileSize);
	startwidth[1] = Math.round(nowscale * (dimension.width - (startpow * settings.tileSize)));
	var maxtile = Math.floor(dimension.width / settings.tileSize);
	startwidth[2] = Math.round(nowscale * (dimension.width - (maxtile * settings.tileSize))); //FB3D Breite/Width des letzten Tiles der Zeile

	$tiles.html('');

	if (typeof(settings.zoomcache[newtile]) == 'undefined') settings.zoomcache[newtile] = {};

	$.each(visibleTiles, function(index, visibleTile) {
		var src = levelDir+visibleTile[0]+'-'+visibleTile[1]+'.jpg'; //FB3D var src = levelDir+'/'+visibleTile[0]+'_'+visibleTile[1]+'.'+settings.format;
		var offsetX = visibleTile[0] == 0 ? 0 : settings.overlap;
		var offsetY = visibleTile[1] == 0 ? 0 : settings.overlap;
		var tilenr = deeplevel+'-'+visibleTile[0]+'-'+visibleTile[1];
		var id = 'zoom-tile-'+tilenr;
		var style = 'position: absolute; left: '+ px(visibleTile[0]*startwidth[0]-offsetX) + '; top: ' + px(visibleTile[1]*startwidth[0]-offsetY) + '; z-index: 0;'; //FB3D option: backface-visibility: hidden; transform: translateZ(0px); transition: opacity 1s linear 0s;
		if (visibleTile[0] != startpow) {
			if (visibleTile[0] == maxtile) style += ' width:' + px(startwidth[2]) + ';'; //FB3D Wenn letztes Teil dann andere Breite
			else style += ' width:' + px(startwidth[0]) + ';';
		} else {
			if (visibleTile[0] == maxtile) style += ' width:' + px(startwidth[2]) + ';'; //FB3D Wenn letztes Teil dann andere Breite
			else style += ' width:' + px(startwidth[1]) + ';';
		}

		if (index == 0) {
			settings.deepzoom_min.left = visibleTile[0];
			settings.deepzoom_min.top = visibleTile[1];
		} else if (index == visibleTiles.length-1) {
			settings.deepzoom_max.left = visibleTile[0];
			settings.deepzoom_max.top = visibleTile[1];
		}
		

		if (settings.zoomcache[newtile][tilenr] !== true) {
			$imgZoom= $(tag(_img_)).attr({ src: src }).appendTo(dot(cache_klass));
			var tileImg = $('<img/>', {
				id: id,
				style: style
			}).appendTo($tiles);
			swapImage(tileImg, src);
			settings.zoomcache[newtile][tilenr] = true;
		} else {
			$('<img/>', {
				src: src,
				id: id,
  ...
  function getDimension(level, settings) {
	if(0 <= level && level < settings.numLevels) {
		...
		var dimension = {};
		dimension.width = number(Math.ceil(settings.zoomwidth * scale));
		dimension.height = number(Math.ceil(settings.zoomheight * scale));
		return dimension;
	}
	else {
  ...
  function getNumTiles(level, settings) {
	if(0 <= level && level < settings.numLevels) {
		var dimension = getDimension(level, settings);
		var cells = {};
		cells.columns = number(Math.ceil(parseFloat(dimension.width) / settings.tileSize));
  		cells.rows = number(Math.ceil(parseFloat(dimension.height) / settings.tileSize));
		return cells;
	}
	else {
  ...
  function getVisibleTiles($cont, settings) {
	var mapX = number($cont.css('left'));
	var mapY = number($cont.css('top'));

	var viewportWidth = settings.imagewidth;
	var viewportHeight = settings.imageheight;
	
	var scaledtileSize = Math.round(settings.tileSize * settings.deepscale);

	var startX, startY, tilesX, tilesY;
	if (mapX < 0) startX = Math.floor(Math.abs(mapX / scaledtileSize));
	else startX = 0;
	if (mapY < 0) startY = Math.floor(Math.abs(mapY / scaledtileSize));
	else startY = 0;
	
	var cells = getNumTiles(settings.deeplevel, settings);
	var tilesX = Math.ceil((viewportWidth - mapX) / scaledtileSize);
	if (tilesX > cells.columns) tilesX = cells.columns;
	var tilesY = Math.ceil((viewportHeight - mapY) / scaledtileSize);
	if (tilesY > cells.rows) tilesY = cells.rows;

	var visibleTileArray = []; 
	var counter = 0;
	for (x = startX; x < tilesX; x++) {
		for (y = startY; y < tilesY; y++) {
  ...
  function setSizePosition($cont, settings, coords ,speed, callback) {
	settings.inAction = true;

	var $holder = settings.holder ? settings.holder : $cont;
	var $thumb = settings.thumb ? settings.thumb : $cont.find('img:first');
	var newtile = settings.frame-1;
	if (settings.changesort) {
		newtile = settings.frames - newtile;
		if (newtile == settings.frames) newtile = 0;
	...
	if (settings.zoomlevel >= 100) {
		...
		levelImage.width = settings.zoomwidth;
		levelImage.height = settings.zoomheight;
		settings.deeplevel = settings.numLevels - 1;
		settings.deepscale = 1;
	} else if (settings.zoomlevel < 1) {
		settings.zoomlevel = settings.startLevel = 0;
		levelImage.width = settings.imagewidth;
		levelImage.height = settings.imageheight;
	} else {
		levelImage.width = Math.round(settings.imagewidth + (settings.zoomwidth - settings.imagewidth)/100 * settings.zoomlevel);
		levelImage.height = Math.round(settings.imageheight + (settings.zoomheight - settings.imageheight)/100 * settings.zoomlevel);

		settings.deeplevel = settings.numLevels - number(Math.log(settings.zoomwidth/levelImage.width)/Math.log(2)) - 1;

		var levelOriginal = getImage(settings.deeplevel, settings);
		settings.deepscale = levelImage.width/levelOriginal.width;

		var testtile = round(settings.deepscale * settings.tileSize);
		settings.deepscale = testtile / settings.tileSize;
		levelImage.width = settings.deepscale * levelOriginal.width;
		levelImage.height = settings.deepscale * levelOriginal.height;
	}
	
	//position
	var pos = {};
	var ratiox = parseFloat(levelImage.width/$holder.width()); //FB3D ratiox/ratioy statt ratio
	var ratioy = parseFloat(levelImage.height/$holder.height());
	var left = number($holder.css('left')== 'auto' ? 0 : $holder.css('left'));
	var top = number($holder.css('top')== 'auto' ? 0 : $holder.css('top'));
	...
	if(coords.left) {
		...
		pos.left = number(viewportWidth / 2) - left;
	}
	//relative center to the event coords ( mousewheel zoom )
		...
		pos.left = number((viewportWidth-levelImage.width) * percentLeft);
	}
	//move center to current center ( + - zoom )
	else {
		var centerX = number(viewportWidth /2) - left;
		pos.left = -number((viewportWidth / -2 ) + centerX * ratiox);
	}

	if(coords.top) {
		var top = levelImage.height / $holder.height() * parseFloat(coords.top);
		pos.top = number(viewportHeight / 2) - top;
	}
	//relative center to the event coords ( mousewheel zoom )
	else if(coords.y){
		...
		pos.top = number((viewportHeight-levelImage.height) * percentTop);
	}
	//move center to current center ( + - zoom )
	else {
		var centerY = number(viewportHeight /2) - top;
		pos.top = -number(number(viewportHeight / -2 ) + centerY * ratioy);
	}

	checkBoundaries($cont, settings, pos);

	if (settings.zoomlevel == 0) {
		var animwidth = settings.imagewidth;
		var animheight = settings.imageheight;
	} else {
		var animwidth = levelImage.width;
		var animheight = levelImage.height;
	}
	...
	var $tiles = $cont.children('div#zoom-tiles');
	$tiles.hide();
	$tiles.css(styles);

	var zoomcontainer = $(dot('zoom-container')).attr('id');
	document.getElementById(zoomcontainer).className= document.getElementById(zoomcontainer).className.replace(/zoom-\d+/, 'zoom-' + settings.zoomlevel);
	...
	$thumb.stop(true,true).animate(styles, speed, "swing", function() {
		if (settings.zoomlevel > 0) $tiles.fadeIn(speed);
  ...
  function checkBoundaries($cont, settings, pos) {
	...
	if(settings.offset.indexOf('%')!==-1) {
		boundaryOffset.x = viewportWidth*number(settings.offset)/100;
		boundaryOffset.y = viewportHeight*number(settings.offset)/100;
	}

	//boundaries
	var minLeft = viewportWidth-levelImage.width-boundaryOffset.x;
	var minTop = viewportHeight-levelImage.height-boundaryOffset.y;
	...
	if(levelImage.width<=viewportWidth) pos.left = number((viewportWidth-levelImage.width)/2);
	if(levelImage.height<=viewportHeight) pos.top = number((viewportHeight-levelImage.height)/2);
